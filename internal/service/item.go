...
func (s *Service) GetItem(ctx context.Context, userID, gameID, economyID int64, itemDefID *big.Int) (*domain.Item, error) {
	...
	item.URI, err = s.blockchain.InventoryRegistry(economy.ContractAddresses.InventoryRegistry).GetItem(item.DefID)
	if err != nil {
...
func (s *Service) CreateItem(ctx context.Context, userID, gameID, economyID int64, form *multipart.Form) (*domain.Item, error) {
	...
	itemdef.DefID, err = s.blockchain.InventoryRegistry(economy.ContractAddresses.InventoryRegistry).CreateItem(itemdef.URI)
	if err != nil {
