...
func main() {
	...
	mc := domain.MasterEconomyContracts{
		...
		Behaviors: domain.MasterBehaviors{
			PurchaseItem: domain.ContractInfo{Address: common.HexToAddress("0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9")},
		},
	}
	...
	itemDefID, err := blockchain.InventoryRegistry(addresses.InventoryRegistry.Address).CreateItem("https://example.com/item-def-1")
	if err != nil {
		log.Fatal().Err(err).Msg("failed to create item definition")
	}
	fmt.Printf("Item Definition ID: %s\n", itemDefID)

	itemDefURI, err := blockchain.InventoryRegistry(addresses.InventoryRegistry.Address).GetItem(itemDefID)
	if err != nil {
